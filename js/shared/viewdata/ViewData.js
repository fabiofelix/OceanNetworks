...
//
//Changed: 05/13/2021
//         Added img_suffix in show_audio object and join_audio_img parametter
//Changed: 06/22/2020
...
//obj = {name: '', color: '', data: [], img_suffix: ''}  
  this.show_audio = function (obj)
  {
    $('#modal_view_data').load(_this.audio_form_path, function(text, status, xhr)
    {
      var suffix = 'img_suffix' in obj ? obj.img_suffix : "";
      var path = _this.join_audio_img ? _this.audio_path : _this.image_path;

      $("#div_audio_title").text(obj.name); 
      $("#audio_spec").attr("src", path + _this.extractImageAudio(obj.name, suffix));  
...
//TODO: Melhorar formatacao dessa apresentacao.
  ...
  this.extractImageAudio = function(audio_name, img_suffix = "", type = ".png")
  {
    var index = audio_name.lastIndexOf(".");
    var name  = audio_name.substring(0, index);
    return name + img_suffix + type;
  };  
